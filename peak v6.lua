-- Novation PEAK preset by @NewIgnis as modified by @kiwigrass for PEAK
-- 18/07/2026
local authorDate = "New Ignis Kiwigrass"
info.setText(authorDate)

local deviceId = 1 -- the device ID used in the E1 preset
local device = devices.get(deviceId)
local port = device:getPort()
local channel = device:getChannel()

-- events to track
events.subscribe(PAGES | POTS)

function preset.onLoad()
  getSettings() -- send the Peak settings sysEx message when the preset loads
end

local w_flag = 0 -- flag for which oscillator's wavetable is in use
local sysExPatch = {} -- to store all sysEx patch data
local sysExSettings = {} -- to store all sysEx settings data 
local sysExWaveName = {} -- to store a user wave name
local nameBank = {}   -- to store patch names of each bank
local defaultVal = {""}
for y = 1,4 do -- 1 = bank A, 2= bank B, 3 = bank C, 4 = bank D
    nameBank[y] = {}
    for z = 0,127 do -- patch numbers
        nameBank[y][z] = defaultVal[1]
    end
end

-- I only want to send some MIDI messages after a control knob is released (for example, to avoid sending a stream of 527 byte sysEx messages when scrolling a settings fader)
-- curCtl holds which of the control knobs was touched (HT @oldgearguy)
curCtl   = -1

-- control Ids for the controls that I only want to send MIDI data after I have released the knob
releaseCtls = {318,356,357,358} -- control # of master clock fader, transpose fader, master fine tune fader, velocity curve 

function events.onPotTouchChange(potId, controlId, touched)
  local idx = get_key_for_value(releaseCtls,controlId)
  local sendHeader = {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x00,0x00,0x00,0x00,0x00,0x00,0x00} -- first section of sysEx message for sending a patch to Peak's edit buffer 
  local settingsHeader = {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x04,0x00,0x00,0x00,0x00,0x00,0x00} -- first section of sysEx message for sending system settings to Peak 
  if (idx == nil) then return end
  if (touched == true) and (curCtl ~= controlId) then curCtl = controlId end
  if (touched == false) and (curCtl == controlId) then
    local midiValue = controls.get(controlId):getValue("value"):getMessage():getValue()
    if curCtl == 318 then -- clock
      sysExPatch[237] = math.floor(midiValue/2)
      sysExPatch[238] = 64*(midiValue%2)
      midi.sendSysex (port, concat(sendHeader,sysExPatch)) -- sends the updated patch to the edit buffer of the Peak
    elseif curCtl == 356 then
      sysExSettings[16] = midiValue
      midi.sendSysex (port, concat(settingsHeader,sysExSettings))  -- sends the updated settings to the Peak
    elseif curCtl == 357 then
      sysExSettings[15] = midiValue
      midi.sendSysex (port, concat(settingsHeader,sysExSettings)) 
    elseif curCtl == 358 then
      sysExSettings[14] = midiValue
      midi.sendSysex (port, concat(settingsHeader,sysExSettings)) 
    end
  end
end

function get_key_for_value(t, value) -- helper function to find match between touched control and target list of controls
   if (t ~= nil) then
      for k,v in pairs(t) do
         if v==value then return k end
      end
   end
   return nil
end

function findArrayByValue(tbl, targetValue1,targetValue2) -- helper function to find target array from a table of arrays
    for i, arr in ipairs(tbl) do
        if arr[1] == targetValue1 and arr[2] == targetValue2 then
            return arr
        end
    end
    return nil -- not found
end

function getBits(value, start, length) -- helper function to get a specific bit series from a value
    -- Calculate the mask by shifting (1 << length) - 1 to the correct position
    local mask = (1 << length) - 1
    -- Shift the mask to the starting position and apply it to the value
    return (value >> (9 - start - length)) & mask
end

function concat(t1,t2) -- helper function to concatenate two arrays. Used for SysEx message construction
  local table = {}
  for i=1,#t1 do
      table[#table+1] = t1[i]
  end
  for i=1,#t2 do
      table[#table+1] = t2[i]
  end
  return table
end

-- display formatter functions ---------------------------------------------------------------------
local notes = {"C  %d","C# %d","D  %d","D# %d","E  %d","F  %d","F# %d","G  %d","G# %d","A  %d","A# %d","B  %d"}

function displayBpm(valueObject,value)
   return(math.floor(value) .. " bpm")
end

function displayNotes (valueObject, value)
  if value > 0 then return (string.format(notes[math.fmod(value-1, 12)+1], (value// 12)-2)) 
  else return("X")
  end
end

function displayPatch (valueObject, value)
  local bankTxt={"A","B","C","D"} -- abbreviations of the bank names
  local bankNum = parameterMap.get (deviceId, PT_VIRTUAL, 6032)
  controls.get (423):setName(nameBank[bankNum][value])  -- display loaded patch names stored in nameBank
--  return (bankTxt[bankNum].." "..string.format("%03d",value))
  return (string.format("%03d",value))
end

function displayChar (valueObject, value) -- borrowed from pro 800 preset to display patch names
  local patchName = ""
  local charNum = 32
  local nameCtrl = 423 -- patch number control id # 
  local patchNum = parameterMap.get (deviceId, PT_VIRTUAL, 6033)
  for i = 0,15 do -- create patch name
    charNum = parameterMap.get (deviceId, PT_VIRTUAL, 10001+i)
    if charNum ~=0 then patchName= patchName..string.char(charNum) end 
  end
  info.setText(string.format("%s",patchName))  -- display name of current patch in bottom right of screen
  controls.get (nameCtrl):setName(patchName)
  return string.char(value)
end

function displayWave (valueObject, value) -- display wavetable name
  return string.char(value)
end

function semiTones(valueObject,value) ---- 
  if value > 0 then 
    return string.format ("%.1f st", value*12/127)
  else
    return string.format ("%.1f st", value*12/128)
  end
end

function percent(valueObject,value) ---- 
  return string.format ("%d %%", value)
end

function st(valueObject,value) ---- 
  return string.format ("%d st", value)
end


function convertToPhase(valueObject,value) ---- 
  return string.format ("%d deg", (value-1)*3)
end

function modWhite(valueObject,value) ---- Set colors of modMatrix according to depth
  local control = valueObject:getControl():getId()
  local color = WHITE -- white
  if value == 0 then  color = 0x202020 end
  controls.get(control-6):setColor (color)
  controls.get(control-5):setColor (color)
  controls.get(control  ):setColor (color)
  controls.get(control+1):setColor (color)
  return string.format ("%d", value)
end

function modOrange(valueObject,value) ---- Set colors of modMatrix according to depth
  local control = valueObject:getControl():getId()
  local color = ORANGE
  if value == 0 then  color = 0x202020 end
  controls.get(control-6):setColor (color)
  controls.get(control-5):setColor (color)
  controls.get(control  ):setColor (color)
  controls.get(control+1):setColor (color)
  return string.format ("%d", value)
end

function modGreen(valueObject,value) ---- Set colors of modMatrix according to depth
  local control = valueObject:getControl():getId()
  local color = GREEN
  if value == 0 then  color = 0x202020 end
  controls.get(control-6):setColor (color)
  controls.get(control-5):setColor (color)
  controls.get(control  ):setColor (color)
  controls.get(control+1):setColor (color)
  return string.format ("%d", value)
end

function lightBlue(valueObject,value) ---- Set colors according to value
  if value == 0 then valueObject:getControl():setColor (0x202067)
    else valueObject:getControl():setColor (BLUE)
  end
  return string.format ("%d", value)
end

function colOrange(valueObject,value) ---- Set colors according to value
  if value == 0 then valueObject:getControl():setColor (0x572700)
   else valueObject:getControl():setColor (ORANGE) --(0xE4660E)
  end
  return string.format ("%d", value)
end

function colGreen(valueObject,value) ---- Set colors according to value
  if value == 0 then valueObject:getControl():setColor (0x003930)
   else valueObject:getControl():setColor (GREEN)
  end
  return string.format ("%d", value)
end

function colYellow(valueObject,value) ---- Set colors according to value
  if value == 0 then valueObject:getControl():setColor (0x505000)
   else valueObject:getControl():setColor (0xF1F50E)
  end
  return string.format ("%d", value)
end

function colWhite(valueObject,value) ---- Set colors according to value
  if value == 0 then valueObject:getControl():setColor (0x202020)
   else valueObject:getControl():setColor (0xFFFFFF)
  end
  return string.format ("%d", value)
end

function colGrey(valueObject,value) ---- Set colors according to value
  if value == 0 then valueObject:getControl():setColor (0x101010)
   else valueObject:getControl():setColor (0x6F6F6F)
  end
  return string.format ("%d", value)
end

function colRed(valueObject,value) ---- Set colors according to value
  if value == 0 then valueObject:getControl():setColor (0x401010)
   else valueObject:getControl():setColor (RED)
  end
  return string.format ("%d", value)
end

function colPurple(valueObject,value) ---- Set colors according to value
  if value == 0 then valueObject:getControl():setColor (0x401040)
   else valueObject:getControl():setColor (PURPLE)
  end
  return string.format ("%d", value)
end

-- user control functions
function waveChange(valueObject, value)
  local parameter = valueObject:getMessage ():getParameterNumber ()
  sendSimpleNRPN(parameter, value)
  local oscNumber = 1
  if parameter == 23 then  oscNumber = 2
    elseif parameter == 32 then oscNumber = 3
  end
  local visiBool = false
  local controlTbl = {{9,38,39,5},{21,50,51,17},{33,62,63,29}} -- shape, wave density, wave spread, wave table
  local control = controls.get(controlTbl[oscNumber][1]) -- setting shape name
  local nameTbl = {"FOLD","FOLD","FOLD","PULSE WIDTH","SHAPE"}
  control:setName(nameTbl[value+1]) 
  if value == 2 then visiBool = true end -- showing wave density and wavespread if saw waveform
  for i = 2,3 do 
    control = controls.get(controlTbl[oscNumber][i]) 
    control:setVisible(visiBool)
  end
  if value == 4 then visiBool = true else visiBool = false end -- shape, wave density, wave spread, wave table
  control = controls.get(controlTbl[oscNumber][4]) 
  control:setVisible(visiBool)
end

function voiceMode(valueObject, value)
  local parameter = valueObject:getMessage ():getParameterNumber ()
  sendSimpleNRPN(parameter, value)
  local controlTbl = {118,130,142} -- eg monotrig
  local visiBool = false
  if value <= 1 then visiBool = true end -- showing monotrig if in mono or monoLG mode
  for i = 1,3 do
    local control = controls.get(controlTbl[i]) 
    control:setVisible(visiBool)
  end
  local controlTbl = {149,161,155,167} -- lfo monotrig, fade sync
  local visiBool = false
  if value <= 2 then visiBool = true end -- showing monotrig if in monophonic mode
  for i = 1,4 do
    local control = controls.get(controlTbl[i]) 
    control:setVisible(visiBool)
  end
  local controlTbl = {156,168} -- lfo common sync
  for i = 1,2 do
    local control = controls.get(controlTbl[i]) 
    control:setVisible(not(visiBool))
  end
end

function unison(valueObject, value) -- 
  local parameter = valueObject:getMessage ():getParameterNumber ()
  sendSimpleNRPN(parameter, value)
  local color = WHITE
  if value == 0 then color = 0x202020 end
  control = controls.get(48) 
  control:setColor (color)
end

function egLoop(valueObject, value)
  local parameter = valueObject:getMessage ():getParameterNumber ()
  sendSimpleNRPN(parameter, value)
  local egNumber = parameter - 3226
  local controlTbl = {119,131,143} -- repeats
  local control = controls.get(controlTbl[egNumber]) 
  local visiBool = false
  if value == 1 then visiBool = true end -- showing repeats if eg in loop
  control:setVisible(visiBool)
end

function glide(valueObject, value) -- 
  local color = GREEN
  if value == 0 then color = 0x001013 end
  for i = 418,419 do 
    controls.get(i):setColor (color)
  end
end

function setSync(valueObject, value)
  local parameter    = valueObject:getMessage ():getParameterNumber ()
  local messageValue = valueObject:getMessage ():getValue ()
  local messageType  = valueObject:getMessage ():getType ()
  if messageType == 0 then sendSimpleNRPN(parameter, messageValue) end
  local paramTbl = {{68,0,146,11,2,147,5,2},{68,1,146,11,2,147,5,2},{68,2,147,11,1,146,5,2},
    {83,0,158,11,8,159,5,14},{83,1,158,11,8,159,5,14},{83,2,159,11,7,158,5,14},
    {3225,0,170,11,14,171,5,26},{3225,1,171,11,13,170,5,26},{3226,0,173,11,20,174,5,29},{3226,1,174,11,19,173,5,29},
    {93,0,195,11,26,194,6,13},{93,1,194,11,25,195,6,13}}  
    -- member array = {parameter, value, ctl to hide, page, slot, ctl to show, page, slot}
    -- 68 = range LFO1, CC31/CC84 = off/on LFO2, 3201-2 = off/on LFO3, 3204-5 = off/on LFO4, CC109/94 = off/on Delay
  local member = findArrayByValue(paramTbl, parameter,value) -- retrieve the desired array
  local control = controls.get(member[3])
  control:setSlot(member[5], member[4])
  local control = controls.get(member[6])
  control:setSlot(member[8], member[7])
end

function simpleNRPN(valueObject, value) -- sends message value, not control value
  local message = valueObject:getMessage ()
  sendSimpleNRPN( message:getParameterNumber (), message:getValue () )
end

function sendSimpleNRPN(nrpn, value)
--  print ("Parameter to send on channel "..channel.." as NRPN = "..nrpn.." with value "..value )
  midi.sendControlChange (port , channel , 99, math.floor(nrpn/128))
  midi.sendControlChange (port , channel , 98, nrpn%128)
  midi.sendControlChange (port , channel , 6, value)
end

function sendPatch(valueObject, value) -- Send patch data
  if value == 0 then return end
  local sendHeader = {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x00,0x00,0x00,0x00,0x00,0x00,0x00}
  midi.sendSysex (port, concat(sendHeader,sysExPatch)) -- sends the current state of the patch from E1 to to edit buffer of the Peak
end

function sendSettings(valueObject, value) -- Send settings data
  if value == 0 then return end
  local sendHeader = {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x04,0x00,0x00,0x00,0x00,0x00,0x00}
  midi.sendSysex (port, concat(sendHeader,sysExSettings)) -- sends the settings back to the Peak
end

-- request functions ---------------------------------------------------------------------------------
function selectIt(valueObject, value)
  if value==0 then return end
  local message = valueObject:getMessage ()
  local parameterNumber = message:getParameterNumber ()
  local messageValue = message:getValue ()
  local bankLSB = parameterMap.get (deviceId, PT_VIRTUAL, parameterNumber-messageValue -1)
  local progNum = parameterMap.get (deviceId, PT_VIRTUAL, parameterNumber-messageValue ) + messageValue - 2
  if bankLSB == 0 then bankLSB =1 end
  if progNum == 128 then progNum= 0 end
  if progNum == -1   then progNum=127  end
  parameterMap.set (deviceId, PT_VIRTUAL, parameterNumber-messageValue,progNum+1)
  parameterMap.set (deviceId, PT_VIRTUAL, parameterNumber-messageValue,progNum)
--  print (string.format("selecting patch: Bank = %d, Prog = %d", bankLSB, progNum))
  midi.sendControlChange(port , channel , 32, bankLSB)
  midi.sendProgramChange(port , channel , progNum)
  midi.sendSysex (port, {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x40,0x00,0x00,0x00,0x00,0x00}) -- requests current patch from edit buffer after program change message sent
end

function getNames(valueObject, value)
  if value == 0 then return end
  local bankNum = parameterMap.get (deviceId, PT_VIRTUAL, 6032)
  midi.sendSysex (port, {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E, 0x41,0x00,0x00,0x00,bankNum,0}) -- requests first patch from bank bankNum
end

function getPatch(valueObject, value) -- Request current patch
  if value == 0 then return end
  midi.sendSysex (port, {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x40,0x00,0x00,0x00,0x00,0x00}) -- requests current patch from edit buffer without a program change message sent first (if patch selected on Peak rather than E1)
end

function getSettings(valueObject,value)
  if value == 0 then return end
  midi.sendSysex (port, {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x44,0x00,0x00,0x00,0x00,0x00}) -- requests Peak settings data
end

function getWaveName(waveNumber)
  midi.sendSysex (port, {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x47,0x00,0x00,waveNumber,0x00,0x00}) -- requests Peak wave name data (user wavetables are 64-73)
end

-- midi processing functions
function assignParam() -- assigns sysEx MIDI values from received patch to corresponding preset parameter values  
  if not sysExPatch[2] then return end -- do not process if not parsed
  local sysEx2Param = {{2,0,10001},{3,0,10002},{4,0,10003},{5,0,10004},{6,0,10005},{7,0,10006},{8,0,10007},{9,0,10008},{10,0,10009},{11,0,10010},{12,0,10011},{13,0,10012},{14,0,10013},{15,0,10014},{16,0,10015},{17,0,10016},{34,0,2},{35,0,3},{36,0,4},
                     {37,0,5},{39,1,5},{40,0,7},{41,1,35},{43,0,9},{44,0,10},{45,0,11},{46,0,12},{47,0,13},{48,1,3},{49,12,14},{51,1,15},{53,1,9},{54,12,16},{56,0,14},{57,0,15},{59,1,12},{60,1,119},{61,1,33},{62,1,34},{63,0,17},{64,0,18},
                     {65,0,19},{66,0,20},{69,1,37},{70,12,17},{72,1,18},{74,1,38},{75,12,19},{77,0,23},{78,0,24},{80,1,39},{81,1,40},{82,1,41},{83,1,42},{84,0,26},{85,0,27},{86,0,28},{87,0,29},{90,1,65},{91,12,20},{93,1,21},{95,1,43},{96,12,22},
                     {98,0,32},{99,0,33},{101,1,71},{102,1,72},{103,1,73},{104,1,44},{105,0,35},{106,0,36},{107,0,37},{108,0,38},{111,12,23},{113,12,24},{115,12,25},{117,12,26},{119,12,27},{121,0,41},{122,0,42},{123,0,43},{124,0,44},{125,1,80},
                     {126,1,36},{127,0,45},{128,0,46},{129,1,75},{130,1,79},{131,12,29},{133,12,28},{135,1,76},{137,1,77},{138,1,78},{139,0,48},{142,0,51},{143,0,52},{146,1,86},{147,1,87},{148,1,88},{149,1,89},{150,0,55},{151,0,56},{152,0,57},
                     {153,0,58},{155,1,90},{156,1,91},{157,1,92},{158,1,93},{159,0,60},{160,0,61},{161,0,62},{162,0,63},{163,1,94},{164,1,95},{165,1,117},{166,1,103},{167,0,64},{168,0,65},{169,0,66},{170,0,67},{171,0,68},{172,12,30},{174,1,81},
                     {175,0,69},{176,0,70},{177,0,71},{178,1,82},{179,0,72},{180,0,73},{181,0,74},{182,0,75},{183,0,76},{185,1,83},{186,12,31},{188,1,84},{189,0,78},{190,0,79},{191,0,80},{192,1,85},{193,0,81},{194,0,82},{195,0,83},{196,0,84},
                     {197,0,85},{199,1,104},{201,0,88},{202,0,89},{204,1,108},{205,1,109},{206,0,91},{207,0,92},{208,0,93},{209,0,94},{210,1,110},{211,0,95},{212,0,96},{213,0,97},{214,0,98},{217,1,112},{219,1,113},{220,0,102},{221,0,103},
                     {222,0,104},{223,0,105},{224,0,106},{225,0,107},{226,0,108},{227,0,109},{229,1,105},{230,0,111},{231,1,118},{232,0,112},{233,1,107},{234,0,113},{235,0,114},{236,0,115},{237,0,16003},{239,0,116},{240,0,117},{241,0,118},{242,0,119},
                     {243,1,116},{244,0,120},{245,0,121},{246,0,122},{247,0,123},{248,0,124},{252,0,128},{253,0,129},{254,0,130},{255,0,131},{256,0,256},{257,0,257},{258,0,258},{259,0,259},{260,0,384},{261,0,385},{262,0,386},{263,0,387},
                     {264,0,512},{265,0,513},{266,0,514},{267,0,515},{268,0,640},{269,0,641},{270,0,642},{271,0,643},{272,0,768},{273,0,769},{274,0,770},{275,0,771},{276,0,896},{277,0,897},{278,0,898},{279,0,899},{280,0,1024},{281,0,1025},
                     {282,0,1026},{283,0,1027},{284,0,1152},{285,0,1153},{286,0,1154},{287,0,1155},{288,0,1280},{289,0,1281},{290,0,1282},{291,0,1283},{292,0,1408},{293,0,1409},{294,0,1410},{295,0,1411},{296,0,1536},{297,0,1537},{298,0,1538},
                     {299,0,1539},{300,0,1664},{301,0,1665},{302,0,1666},{303,0,1667},{304,0,1792},{305,0,1793},{306,0,1794},{307,0,1795},{308,0,1920},{309,0,1921},{310,0,1922},{311,0,1923},{312,0,2048},{313,0,2049},{314,0,2050},{315,0,2051},
                     {316,0,2176},{317,0,2177},{318,0,2178},{319,0,2179},{320,0,2304},{321,0,2305},{322,0,2306},{323,0,2307},{324,0,2432},{325,0,2433},{326,0,2434},{327,0,2435},{328,0,2560},{329,0,2561},{330,0,2562},{331,0,2563},{348,0,3200},
                     {349,0,3201},{350,0,3202},{351,0,3203},{352,0,3204},{353,0,3205},{354,0,3206},{357,0,3209},{358,0,3210},{359,0,3211},{361,0,3213},{362,0,3214},{363,0,3215},{365,0,3217},{366,0,3218},{367,0,3219},{369,0,3221},{370,0,3222},{371,0,3223},
                     {373,0,3225},{374,0,3226},{375,0,3227},{376,0,3228},{377,0,3229},{378,0,3230},{379,0,3231},{380,0,3232},{381,0,3233},{382,0,3234},{383,0,3235},{384,0,3236},{385,0,3237},{386,0,3238},{387,0,3239},{388,0,3240},{389,0,3241},
                     {390,0,3242},{391,0,3243},{392,0,3244},{393,0,3245}}
  for i = 1, #sysEx2Param do
    local sysExByte = sysEx2Param[i][1] -- byte number of sysEx payload 
    local parType = sysEx2Param[i][2]%10 -- parameter type (0 = virtual, 1 = CC7, 2 = CC14 -- see below for value calculation, 3 = NRPN -- not used)
    local parNum = sysEx2Param[i][3]  -- parameter number
    local value = sysExPatch[sysExByte] -- parameter value
    if sysEx2Param[i][2] == 12 then -- 12 stands for a type CC14 of which byte 1 has bits 8-1 and byte 2 has bit 0 on place 6
      value = 64 * (sysExPatch[sysEx2Param[i][1]] * 2 + getBits(sysExPatch[sysEx2Param[i][1]+1], 2, 1) )
    elseif sysEx2Param[i][1] == 237 then  -- clock rate 
      value = (sysExPatch[sysEx2Param[i][1]] * 2 + getBits(sysExPatch[sysEx2Param[i][1]+1], 2, 1) )
      --print("i "..i..", byte "..sysEx2Param[i][1].. ": value = "..value.."  bit 0 = "..getBits(sysExPatch[partNum][sysEx2Param[i][1]+1],2,1) )
    end
    parameterMap.set (deviceId, parType, parNum, value)
  end
end

function assignSettings() -- assigns sysEx MIDI values from received settings to corresponding preset parameter values  
  if not sysExSettings[2] then return end -- do not process if not parsed
  local param = {{2,8192},{8,8198},{14,8204},{15,8205},{16,8206},{19,8209},{31,8221},{32,8222}}
  for i = 1, #param do
    parameterMap.set (deviceId, 0, param[i][2], sysExSettings[param[i][1]])
  end
end

function assignWaveName() -- assigns sysEx MIDI values from received wave name to corresponding preset parameter values  
  if not sysExWaveName[2] then print("Nope") return end -- do not process if not parsed
  local param = {{1,11001},{2,11002},{3,11003},{4,11004},{5,11005},{6,11006},{7,11007}}
  for i = 1, #param do
    parameterMap.set (deviceId, 0, param[i][2], sysExWaveName[param[i][1]])
  end
end

-- MIDI callback functions ----------------------------------------------------------------------------------------------------------------
function midi.onSysex(midiInput, sysexBlock)
  local charNum = 32
  local waveName = ""
  -- Peak patch bank sent
  if sysexBlock:getLength() > 33 and sysexBlock:peek(1) == 0xF0 and sysexBlock:peek(2) == 0x00 and sysexBlock:peek(3) == 0x20
    and sysexBlock:peek(4) == 0x29 and sysexBlock:peek(5) == 0x01 and sysexBlock:peek(6) == 0x10 and sysexBlock:peek(7) == 0x00
    and sysexBlock:peek(8) == 0x7E and sysexBlock:peek(9) == 0x01     
  then 
    local bankNum = sysexBlock:peek(13)
    local patchNum = sysexBlock:peek(14)
    nameBank[bankNum][patchNum] = ""
    for i = 0,15 do -- read patch name
      nameBank[bankNum][patchNum]= nameBank[bankNum][patchNum]..string.char(sysexBlock:peek(17+i))
    end 
    if patchNum < 127 then -- get next patch
      midi.sendSysex (port, {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E, 0x41,0x00,0x00,0x00,bankNum,patchNum+1}) -- request patch patchNum+1 from bank bankNum
    else
      parameterMap.set (deviceId, PT_VIRTUAL, 16004,0) -- stop retrieving patch names for bank
    end
  -- patch from edit buffer sent
  elseif sysexBlock:getLength() == 527 and sysexBlock:peek(1) == 0xF0 and sysexBlock:peek(2) == 0x00 and sysexBlock:peek(3) == 0x20
    and sysexBlock:peek(4) == 0x29 and sysexBlock:peek(5) == 0x01 and sysexBlock:peek(6) == 0x10 and sysexBlock:peek(7) == 0x00
    and sysexBlock:peek(8) == 0x7E and sysexBlock:peek(9) == 0x00    
  then 
    for i = 1,511 do
      sysExPatch[i]=sysexBlock:peek(i+16-1)
    end
    assignParam()
  -- Peak settings sent
  elseif sysexBlock:getLength() == 527 and sysexBlock:peek(1) == 0xF0 and sysexBlock:peek(2) == 0x00 and sysexBlock:peek(3) == 0x20
    and sysexBlock:peek(4) == 0x29 and sysexBlock:peek(5) == 0x01 and sysexBlock:peek(6) == 0x10 and sysexBlock:peek(7) == 0x00
    and sysexBlock:peek(8) == 0x7E and sysexBlock:peek(9) == 0x04    
  then 
    for i = 1,511 do
      sysExSettings[i]=sysexBlock:peek(i+16-1)
    end
    assignSettings()
  -- Peak user wavetable name sent
  elseif sysexBlock:getLength() == 23 and sysexBlock:peek(1) == 0xF0 and sysexBlock:peek(2) == 0x00 and sysexBlock:peek(3) == 0x20
    and sysexBlock:peek(4) == 0x29 and sysexBlock:peek(5) == 0x01 and sysexBlock:peek(6) == 0x10 and sysexBlock:peek(7) == 0x00
    and sysexBlock:peek(8) == 0x7E and sysexBlock:peek(9) == 0x07  
  then 
    for i = 1,7 do
      sysExWaveName[i]=sysexBlock:peek(i+16-1)
    end
    for i = 1,7 do -- create wave name
      charNum = sysExWaveName[i]
      if charNum ~=0 then waveName = waveName..string.char(charNum) end 
    end
--    print(waveName)
    if w_flag == 1 then -- replace control name with user wavetable name if user wavetable has been selected for osc 1 and reset the wavetable flag to 0
      controls.get (5):setName(waveName)
      w_flag = 0
    end
    if w_flag == 2 then -- replace control name with user wavetable name if user wavetable has been selected for osc 2 and reset the wavetable flag to 0
      controls.get (17):setName(waveName)
      w_flag = 0
    end
    if w_flag == 3 then -- replace control name with user wavetable name if user wavetable has been selected for osc 3 and reset the wavetable flag to 0
      controls.get (29):setName(waveName)
      w_flag = 0
    end
  end
end

function midi.onControlChange(midiInput, channel, controllerNumber, value)
  if controllerNumber == 32 then -- bankLSB 
    parameterMap.set(deviceId, PT_VIRTUAL, 6032, value) -- updates bank number parameter if change bank on the Peak
  elseif controllerNumber == 99 then -- NRPN MSB number
    msbNum = math.floor(value)
  elseif controllerNumber == 98 then -- NRPN LSB number
    lsbNum = math.floor(value)
  elseif controllerNumber == 6 then -- NRPN MSB value
    msbVal = math.floor(value)
    parameterMap.set(deviceId, PT_VIRTUAL, 128*msbNum + lsbNum, msbVal) -- update the associated virtual parameter if there is a change in an NRPN on the peak 
  end
end

function midi.onProgramChange(midiInput, channel, programNumber)
  parameterMap.set(deviceId, PT_VIRTUAL, 6033, programNumber) -- updates patch number parameter if change patch on the Peak
end

function parameterMap.onChange(valueObjects, origin, midiValue)
  if (origin ~= INTERNAL) then return end
  local ctlId = valueObjects[1]:getControl():getId()
  local pNum = valueObjects[1]:getMessage():getParameterNumber()
  local settingsHeader = {0x00, 0x20, 0x29, 0x01, 0x10, 0x00, 0x7E,0x04,0x00,0x00,0x00,0x00,0x00,0x00} -- first section of sysEx message for sending system settings to Peak 
--  local waveName = ""
  local charNum = 32
  if pNum == 8192 then -- Patch cue toggle
    sysExSettings[2] = parameterMap.get(deviceId, PT_VIRTUAL, pNum)
    midi.sendSysex (port, concat(settingsHeader,sysExSettings)) -- sends the updated settings to the Peak
  elseif pNum == 8198 then -- clock source
    sysExSettings[8] = parameterMap.get(deviceId, PT_VIRTUAL, pNum)
    midi.sendSysex (port, concat(settingsHeader,sysExSettings)) 
  elseif pNum == 8209 then -- ARP > MIDI toggle
    sysExSettings[19] = parameterMap.get(deviceId, PT_VIRTUAL, pNum)
    midi.sendSysex (port, concat(settingsHeader,sysExSettings)) 
  elseif pNum == 8221 then -- master vol range
    sysExSettings[31] = parameterMap.get(deviceId, PT_VIRTUAL, pNum)
    midi.sendSysex (port, concat(settingsHeader,sysExSettings)) 
  elseif pNum == 8222 then -- tuning mode for tuning table 
    sysExSettings[32] = parameterMap.get(deviceId, PT_VIRTUAL, pNum)
    midi.sendSysex (port, concat(settingsHeader,sysExSettings)) 
  elseif (pNum == 15 and midiValue > 63) then -- retrieve user wavetable name for osc 1 and set wavetable flag to 1 
      getWaveName(midiValue)
      w_flag = 1
  elseif (pNum == 24 and midiValue > 63) then -- retrieve user wavetable name for osc 2 and set wavetable flag to 2
      getWaveName(midiValue)
      w_flag = 2
  elseif (pNum == 33 and midiValue > 63) then -- retrieve user wavetable name for osc 3 and set wavetable flag to 3
      getWaveName(midiValue)
      w_flag = 3
  elseif (pNum == 15 and midiValue < 64) then -- use default control name if not using a user wavetable
      controls.get (5):setName("WAVE TABLE")
  elseif (pNum == 24 and midiValue < 64) then 
      controls.get (17):setName("WAVE TABLE")
  elseif (pNum == 33 and midiValue < 64) then 
      controls.get (29):setName("WAVE TABLE")
  end
end
