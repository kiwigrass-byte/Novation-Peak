...
function currentMulti(valueObject, value)
  if value == 0 then return end
  midi.sendSysex(globalPort, concat(PEAK_SYSEX, {0x42,0x00,0x00,0x00,0x00,0x00}) )
end

function getGlobalSettings(valueObject, value)
  if value == 0 then return end
  -- Monitor console on first load to verify response arrives (cmd=0x04 in onSysex).
  midi.sendSysex(globalPort, concat(PEAK_SYSEX, {0x44,0x00,0x00,0x00,0x00,0x00}) )
end
...
function getAllWaveNames(valueObject, value)
  if value == 0 then return end
  for i = 64, 73 do
    midi.sendSysex(globalPort, concat(PEAK_SYSEX, {0x47,0x00,0x00,i,0x00,0x00}) )
  end
end
...
function sendPatch(valueObject, value)
  ...
  if value == 0 then return end
  local sendHeader = concat(PEAK_SYSEX,{0x00,0x00,0x00,0x00,0x00,0x00,0x00} )
