...
function preset.onLoad()
  getAllWaveNames(nil, 1) -- request Peak Wavetable names
  getSettings(nil,1) -- request Peak settings when the preset loads
end
...
function displayPatch (valueObject, value)
  ...
  controls.get (423):setName(patchName)  -- display loaded patch names stored in nameBank
...
function displayChar (valueObject, value) -- borrowed from pro 800 preset to display patch names
  ...
  local patchNum = parameterMap.get (deviceId, PT_VIRTUAL, 10033)
...
-- Query current scanner status (not used)
...
function midi.onSysex(midiInput, sysexBlock)
  ...
  if len > 33 and cmd == 0x01 then 
    ...
    print("bank ".. bankNum.."  patch " ..patchNum.." received")
  ...
  print("user wavetable names received") 
    local waveName = ""
    for i = 1, 7 do 
